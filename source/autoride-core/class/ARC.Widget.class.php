...
class ARCWidget extends WP_Widget 
{
	...
	function widget($argument,$instance)
	{
		...
		$Template=new Autoride_ThemeTemplate($data,PLUGIN_ARC_TEMPLATE_PATH.$argument['_data']['file']);
	...
	function form($instance)
	{
		...
		if(is_array($instance['_data']['field']))
		{
			...
			{
				Autoride_ThemeHelper::removeUIndex($instance,$value);
				Autoride_ThemeHelper::removeUIndex($data['option'][$value],'id','name','value');
		...
		$Template=new Autoride_ThemeTemplate($data,PLUGIN_ARC_TEMPLATE_PATH.'admin/'.$instance['_data']['file']);
